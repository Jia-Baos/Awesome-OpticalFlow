                                ...
                                const float *__restrict__ add, int d, float mul)
{
  ...
  else
  {
    ...
    long int inc = 1;
    long int d_tmp = d;
    saxpy_(&d_tmp, &mul, (float *)add, &inc, (float *)dest, &inc);
  }
}
                                    ...
                                    int dx, int dy, const int tx, const int ty, const int ex, const float def)
{
  ...
  if (dx > tx)
    dx = tx; // after those alues, nothing happens anyway
  if (dy > ty)
    dy = ty;
  if (-dx > tx)
    dx = -tx;
...
#define add_default(n)                 \
  {                                    \
    fast_set_val(dest, (n), mul *def); \
    dest += (n);                       \
  }
  float *_dest = dest;
  ...
  for (j = MAX(0, dy); j < MIN(ty, ty + dy + ex); j++)
  {
    ...
    if (dx >= 0)
    {
      add_default(dx) if (ex) add_default(ex)
    }
  }
  ...
  if (dy >= 0)
  {
    add_default(dy * (tx + ex)) if (ex) add_default(ex * (tx + ex))
  }
...
static inline int retrieve_children(const int x, const int y, const int_cube *child_grid)
{
  ...
  const int step0 = child_grid->tx == 1 && child_grid->ty == 1 ? 1 : MAX(child_grid->pixels[2] - child_grid->pixels[0], child_grid->pixels[1 + 2 * child_grid->tx] - child_grid->pixels[1]);
  ...
  if (i < 0 || i >= child_grid->tx)
    return -1;
  if (j < 0 || j >= child_grid->ty)
    return -1;
...
// extern "C"
// {
//   int sgemm_(char *transa, char *transb, integer *m, integer *n, integer *k, float *alpha, float *a, integer *lda, float *b, integer *ldb, float *beta, float *c, integer *ldc);
// }
...
#pragma omp parallel for num_threads(n_thread)
  ...
  for (l = 0; l < n_thread; l++)
  {
    ...
    // Jia-Baos
    // 2024-01-27 00:05
    long int npix_tmp = npix;
    long int np_tmp = np;
    long int nh_tmp = nh;
    sgemm_(&T, &T, &npix_tmp, &np_tmp, &nh_tmp, &alpha,
           convolved_hog->pixels, &npix_tmp,
           p, &nh_tmp, &beta, r, &npix_tmp);
  }
}
...
#pragma omp parallel for num_threads(n_thread)
  ...
  for (int j = 0; j < res_ty; ++j)
  {
    ...
    for (int l = 0; l < P; l++)
    {
      ...
      // 2024-01-27 00:05
      long int nh_tmp = nh;
      long int res_tx_tmp = res_tx;
      long int one_tmp = one;
      // blas fast matrix-vector product
      sgemv_(&T, &nh_tmp, &res_tx_tmp, &alpha, c + j * tx * nh, &nh_tmp,
             p, &one_tmp, &beta, r + j * res_tx, &one_tmp);
    }
  }
}
