  ...
  float tangent(const float x1, const float y1, const float x2, const float y2)
  {
    ...
    if (yDiff > 0)
    {
      if (xDiff == 0)
        ...
        alpha = atan(yDiff / xDiff);
      else
        alpha = Pi + atan(yDiff / xDiff);
    }
    else
    {
      if (xDiff == 0)
  ...
  void PATransformation(const CMatrix<float> &aMatrix, CVector<float> &aEigenvalues, CMatrix<float> &aEigenvectors, bool aOrdering)
  {
        ...
        for (int p = 0; p < n - 1; p++)
          for (int q = p + 1; q < n; q++)
            if (fabs(aCopy(q, p)) >= eps2)
            {
              ...
              t = 1.0;
              if (fabs(theta) > delta)
                t = 1.0 / (theta + theta / fabs(theta) * sqrt(theta * theta + 1.0));
              c = 1.0 / sqrt(1.0 + t * t);
              s = c * t;
              r = s / (1.0 + c);
    ...
    if (aOrdering)
    {
      ...
      for (int i = 0; i < n - 1; i++)
      {
        int k = i;
        for (int j = i + 1; j < n; j++)
          if (fabs(aEigenvalues(j)) > fabs(aEigenvalues(k)))
            k = j;
        if (k != i)
        {
...
#define SIGN(a, b) ((b) >= 0.0 ? fabs(a) : -fabs(a))
    ...
    for (k = aXSize - 1; k >= 0; k--)
    {
      for (its = 1; its <= aIterations; its++)
      {
        ...
        if (its == aIterations)
          std::cerr << "svd: No convergence in " << aIterations << " iterations" << std::endl;
        x = S(l, l);
        nm = k - 1;
        y = S(nm, nm);
        g = aBuffer(nm);
...
#undef SIGN
  ...
  {
    if (A.xSize() != A.ySize())
      throw ENonquadraticMatrix(A.xSize(), A.ySize());
  ...
  void cholesky(CMatrix<float> &A)
  {
    ...
    for (int i = 0; i < A.xSize(); i++)
      for (int j = i; j < A.ySize(); j++)
      {
        ...
        {
          if (sum <= 0.0)
            return; // throw ENonPositiveDefinite();
          d(i) = sqrt(sum);
        }
        else
          A(i, j) = sum / d(i);
  ...
  void RBM2Twist(CVector<float> &T, CMatrix<float> &fRBM)
  {
    ...
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < 3; j++)
        if (i != j)
          sum += dRBM(i, j) * dRBM(i, j);
        else
          sum += (dRBM(i, i) - 1.0) * (dRBM(i, i) - 1.0);
    if (sum < 0.0000001)
    {
      T(0) = fRBM(3, 0);
      T(1) = fRBM(3, 1);
    ...
    else
    {
      ...
      omegaHat.data()[0] = 0.0;
      omegaHat.data()[1] = -omega(2);
      omegaHat.data()[2] = omega(1);
      omegaHat.data()[3] = omega(2);
      omegaHat.data()[4] = 0.0;
      omegaHat.data()[5] = -omega(0);
      omegaHat.data()[6] = -omega(1);
      omegaHat.data()[7] = omega(0);
      omegaHat.data()[8] = 0.0;
