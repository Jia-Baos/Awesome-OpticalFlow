...
namespace NFilter
{
  ...
  template <class T>
  void osher(CMatrix<T> &aData, int aIterations = 20);
  template <class T>
  inline void osher(const CMatrix<T> &aData, CMatrix<T> &aResult, int aIterations = 20);
}
...
namespace NFilter
{
  ...
  void filter(const CVector<T> &aVector, CVector<T> &aResult, const CFilter<T> &aFilter)
  {
    if (aResult.size() != aVector.size())
      throw EFilterIncompatibleSize(aVector.size(), aResult.size());
    int x1 = -aFilter.A();
    ...
    for (int i = 0; i < x1; i++)
    {
      ...
      for (int j = aFilter.A(); j < aFilter.B(); j++)
        if (j + i < 0)
          aResult(i) += aFilter(j) * aVector(-1 - j - i);
        else
    ...
    for (int i = x2; i < aResult.size(); i++)
    {
      ...
      for (int j = aFilter.A(); j < aFilter.B(); j++)
        if (j + i >= aVector.size())
          aResult(i) += aFilter(j) * aVector(a2Size - j - i);
        else
          aResult(i) += aFilter(j) * aVector(j + i);
    }
  }
  ...
  void boxFilter(const CVector<T> &aVector, CVector<T> &aResult, int aWidth)
  {
    ...
    if (aRight >= aVector.size())
      aRight = aVector.size() - 1;
    // Initialize
    T aSum = 0.0f;
    ...
    for (int x = 0; x < aVector.size(); x++)
    {
      ...
      if (x - halfWidth >= 0)
      {
        aSum -= aVector(x - halfWidth);
        aNum--;
      }
      if (x + halfWidth + 1 < aVector.size())
      {
        aSum += aVector(x + halfWidth + 1);
        aNum++;
      }
    }
    delete[] invWidth;
  ...
  void filter(const CMatrix<T> &aMatrix, CMatrix<T> &aResult, const CFilter<T> &aFilter, const int aDummy)
  {
    ...
    for (int y = 0; y < aMatrix.ySize(); y++)
    {
      ...
      for (int x = 0; x < x1; x++)
        ...
        {
          if (x + i < 0)
            aResult.data()[aOffset + x] += aFilter[i] * aMatrix.data()[aOffset - 1 - x - i];
          else if (x + i >= aMatrix.xSize())
            aResult.data()[aOffset + x] += aFilter[i] * aMatrix.data()[aOffset + a2Size - x - i];
          else
  ...
  void filter(const CMatrix<T> &aMatrix, CMatrix<T> &aResult, const int aDummy, const CFilter<T> &aFilter)
  {
    ...
    for (int y = 0; y < y1; y++)
      for (int x = 0; x < aMatrix.xSize(); x++)
      {
        ...
        {
          if (y + j < 0)
            aResult(x, y) += aFilter[j] * aMatrix(x, -1 - y - j);
          else if (y + j >= aMatrix.ySize())
            aResult(x, y) += aFilter[j] * aMatrix(x, a2Size - y - j);
          else
    ...
    for (int y = 0; y < y1; y++)
      for (int x = 0; x < aMatrix.xSize(); x++)
      {
        ...
        for (int j = aFilter.AY(); j < aFilter.BY(); j++)
        {
          ...
          {
            if (x + i < 0)
              aResult(x, y) += aFilter(i, j) * aMatrix(-1 - x - i, tempY);
            else if (x + i >= aMatrix.xSize())
              aResult(x, y) += aFilter(i, j) * aMatrix(a2XSize - x - i, tempY);
            else
              aResult(x, y) += aFilter(i, j) * aMatrix(x + i, tempY);
          }
        }
      }
    // Lower rim
    ...
    for (int y = y1; y < y2; y++)
    {
      ...
      for (int x = 0; x < x1; x++)
      {
        ...
        for (int j = aFilter.AY(); j < aFilter.BY(); j++)
        {
          ...
          {
            if (x + i < 0)
              aResult(x, y) += aFilter(i, j) * aMatrix(-1 - x - i, y + j);
            else if (x + i >= aMatrix.xSize())
              aResult(x, y) += aFilter(i, j) * aMatrix(a2XSize - x - i, y + j);
            else
  ...
  void boxFilterX(const CMatrix<T> &aMatrix, CMatrix<T> &aResult, int aWidth)
  {
    ...
    for (int y = 0; y < aMatrix.ySize(); y++)
    {
      ...
      for (int x = 0; x < aMatrix.xSize(); x++)
      {
        ...
        if (x - halfWidth >= 0)
        {
          aSum -= aMatrix.data()[aOffset + x - halfWidth];
          aNum--;
        }
        if (x + halfWidth + 1 < aMatrix.xSize())
        {
          aSum += aMatrix.data()[aOffset + x + halfWidth + 1];
          aNum++;
        }
      }
    }
  ...
  void boxFilterY(const CMatrix<T> &aMatrix, CMatrix<T> &aResult, int aWidth)
  {
    ...
    for (int x = 0; x < aMatrix.xSize(); x++)
    {
      ...
      for (int y = 0; y < aMatrix.ySize(); y++)
      {
        aResult(x, y) = aSum * invWidth[aNum];
        if (y - halfWidth >= 0)
        {
          aSum -= aMatrix(x, y - halfWidth);
          aNum--;
        }
        if (y + halfWidth + 1 < aMatrix.ySize())
        {
          aSum += aMatrix(x, y + halfWidth + 1);
          aNum++;
        }
      }
    }
  ...
  void recursiveSmoothX(CMatrix<T> &aMatrix, float aSigma)
  {
    ...
    for (int y = 0; y < aMatrix.ySize(); y++)
    {
      aVals1(0) = 0.5 * aMatrix(0, y);
      aVals1(1) = k * (aMatrix(1, y) + aPreMinus * aMatrix(0, y)) + (2.0 * aExp - aExpSqr) * aVals1(0);
      for (int x = 2; x < aMatrix.xSize(); x++)
        aVals1(x) = k * (aMatrix(x, y) + aPreMinus * aMatrix(x - 1, y)) + a2Exp * aVals1(x - 1) - aExpSqr * aVals1(x - 2);
      aVals2(aMatrix.xSize() - 1) = 0.5 * aMatrix(aMatrix.xSize() - 1, y);
  ...
  void recursiveSmoothY(CMatrix<T> &aMatrix, float aSigma)
  {
    ...
    for (int x = 0; x < aMatrix.xSize(); x++)
    {
      aVals1(0) = 0.5 * aMatrix(x, 0);
      aVals1(1) = k * (aMatrix(x, 1) + aPreMinus * aMatrix(x, 0)) + (2.0 * aExp - aExpSqr) * aVals1(0);
      for (int y = 2; y < aMatrix.ySize(); y++)
        aVals1(y) = k * (aMatrix(x, y) + aPreMinus * aMatrix(x, y - 1)) + a2Exp * aVals1(y - 1) - aExpSqr * aVals1(y - 2);
      aVals2(aMatrix.ySize() - 1) = 0.5 * aMatrix(x, aMatrix.ySize() - 1);
    ...
    for (int z = 0; z < aTensor.zSize(); z++)
      for (int y = 0; y < aTensor.ySize(); y++)
      {
        ...
        for (int x = 0; x < x1; x++)
        {
          ...
          {
            if (x + i < 0)
              aResult(x, y, z) += aFilter[i] * aTensor(-1 - x - i, y, z);
            else if (x + i >= aTensor.xSize())
              aResult(x, y, z) += aFilter[i] * aTensor(a2Size - x - i, y, z);
            else
    ...
    for (int z = 0; z < aTensor.zSize(); z++)
    {
      ...
      for (int y = 0; y < y1; y++)
        for (int x = 0; x < aTensor.xSize(); x++)
        {
          ...
          {
            if (y + i < 0)
              aResult(x, y, z) += aFilter[i] * aTensor(x, -1 - y - i, z);
            else if (y + i >= aTensor.ySize())
              aResult(x, y, z) += aFilter[i] * aTensor(x, a2Size - y - i, z);
            else
    ...
    for (int z = 0; z < z1; z++)
      for (int y = 0; y < aTensor.ySize(); y++)
        for (int x = 0; x < aTensor.xSize(); x++)
        {
          ...
          {
            if (z + i < 0)
              aResult(x, y, z) += aFilter[i] * aTensor(x, y, -1 - z - i);
            else if (z + i >= aTensor.zSize())
              aResult(x, y, z) += aFilter[i] * aTensor(x, y, a2Size - z - i);
            else
  ...
  void boxFilterX(const CTensor<T> &aTensor, CTensor<T> &aResult, int aWidth)
  {
    ...
    int halfWidth = (aWidth >> 1);
    int aRight = halfWidth;
    if (aRight >= aTensor.xSize())
      aRight = aTensor.xSize() - 1;
    for (int z = 0; z < aTensor.zSize(); z++)
      for (int y = 0; y < aTensor.ySize(); y++)
      {
        ...
        for (int x = 0; x < aTensor.xSize(); x++)
        {
          ...
          if (x - halfWidth >= 0)
          {
            aSum -= aTensor.data()[aOffset + x - halfWidth];
            aNum--;
          }
          if (x + halfWidth + 1 < aTensor.xSize())
          {
            aSum += aTensor.data()[aOffset + x + halfWidth + 1];
            aNum++;
          }
        }
      }
  ...
  void boxFilterY(const CTensor<T> &aTensor, CTensor<T> &aResult, int aWidth)
  {
    ...
    int halfWidth = (aWidth >> 1);
    int aBottom = halfWidth;
    if (aBottom >= aTensor.ySize())
      aBottom = aTensor.ySize() - 1;
    for (int z = 0; z < aTensor.zSize(); z++)
      for (int x = 0; x < aTensor.xSize(); x++)
      {
        // Initialize
        ...
        for (int y = 0; y < aTensor.ySize(); y++)
        {
          ...
          if (y - halfWidth >= 0)
          {
            aSum -= aTensor(x, y - halfWidth, z);
            aNum--;
          }
          if (y + halfWidth + 1 < aTensor.ySize())
          {
            aSum += aTensor(x, y + halfWidth + 1, z);
            aNum++;
          }
        }
      }
  ...
  void boxFilterZ(const CTensor<T> &aTensor, CTensor<T> &aResult, int aWidth)
  {
    ...
    int halfWidth = (aWidth >> 1);
    int aBottom = halfWidth;
    if (aBottom >= aTensor.zSize())
      aBottom = aTensor.zSize() - 1;
    for (int y = 0; y < aTensor.ySize(); y++)
      for (int x = 0; x < aTensor.xSize(); x++)
      {
        // Initialize
        ...
        for (int z = 0; z < aTensor.zSize(); z++)
        {
          ...
          if (z - halfWidth >= 0)
          {
            aSum -= aTensor(x, y, z - halfWidth);
            aNum--;
          }
          if (z + halfWidth + 1 < aTensor.zSize())
          {
            aSum += aTensor(x, y, z + halfWidth + 1);
            aNum++;
          }
        }
      }
  ...
  void recursiveSmoothX(CTensor<T> &aTensor, float aSigma)
  {
    ...
    for (int z = 0; z < aTensor.zSize(); z++)
      ...
      {
        int aOffset = (z * aTensor.ySize() + y) * aTensor.xSize();
        aVals1(0) = 0.5 * aTensor.data()[aOffset];
        ...
        aVals2(aTensor.xSize() - 1) = 0.5 * aTensor.data()[aOffset + aTensor.xSize() - 1];
  ...
  void recursiveSmoothY(CTensor<T> &aTensor, float aSigma)
  {
    ...
    for (int z = 0; z < aTensor.zSize(); z++)
      for (int x = 0; x < aTensor.xSize(); x++)
      {
        ...
        aVals2(aTensor.ySize() - 1) = 0.5 * aTensor(x, aTensor.ySize() - 1, z);
  ...
  void recursiveSmoothZ(CTensor<T> &aTensor, float aSigma)
  {
    ...
    for (int y = 0; y < aTensor.ySize(); y++)
      for (int x = 0; x < aTensor.xSize(); x++)
      {
        ...
        aVals2(aTensor.zSize() - 1) = 0.5 * aTensor(x, y, aTensor.zSize() - 1);
      ...
      for (int z = 0; z < aTensor.zSize(); z++)
        for (int y = 0; y < aTensor.ySize(); y++)
        {
          ...
          for (int x = 0; x < x1; x++)
            ...
            {
              if (x + i < 0)
                aResult.data()[aOffset + x] += aFilter[i] * aTensor.data()[aOffset - 1 - x - i];
              else if (x + i >= aTensor.xSize())
                aResult.data()[aOffset + x] += aFilter[i] * aTensor.data()[aOffset + a2Size - x - i];
              else
                aResult.data()[aOffset + x] += aFilter[i] * aTensor.data()[x + i + aOffset];
            }
          // Center
          for (int x = x1; x < x2; x++)
            for (int i = aFilter.A(); i < aFilter.B(); i++)
      ...
      for (int z = 0; z < aTensor.zSize(); z++)
      {
        ...
        for (int y = 0; y < y1; y++)
          for (int x = 0; x < aTensor.xSize(); x++)
            ...
            {
              if (y + i < 0)
                aResult(x, y, z, a) += aFilter[i] * aTensor(x, -1 - y - i, z, a);
              else if (y + i >= aTensor.ySize())
                aResult(x, y, z, a) += aFilter[i] * aTensor(x, a2Size - y - i, z, a);
              else
      ...
      for (int z = 0; z < z1; z++)
        for (int y = 0; y < aTensor.ySize(); y++)
          for (int x = 0; x < aTensor.xSize(); x++)
            ...
            {
              if (z + i < 0)
                aResult(x, y, z, a) += aFilter[i] * aTensor(x, y, -1 - z - i, a);
              else if (z + i >= aTensor.zSize())
                aResult(x, y, z, a) += aFilter[i] * aTensor(x, y, a2Size - z - i, a);
              else
      ...
      for (int z = 0; z < aTensor.zSize(); z++)
        for (int y = 0; y < aTensor.ySize(); y++)
          for (int x = 0; x < aTensor.xSize(); x++)
            ...
            {
              if (a + i < 0)
                aResult(x, y, z, a) += aFilter[i] * aTensor(x, y, z, -1 - a - i);
              else if (a + i >= aTensor.aSize())
                aResult(x, y, z, a) += aFilter[i] * aTensor(x, y, z, a2Size - a - i);
              else
    ...
    for (int a = 0; a < aTensor.aSize(); a++)
      for (int z = 0; z < aTensor.zSize(); z++)
        for (int x = 0; x < aTensor.xSize(); x++)
        {
          ...
          aVals2(aTensor.ySize() - 1) = 0.5 * aVals3(aTensor.ySize() - 1);
    ...
    for (int a = 0; a < aTensor.aSize(); a++)
      for (int y = 0; y < aTensor.ySize(); y++)
        for (int x = 0; x < aTensor.xSize(); x++)
        {
          ...
          aVals2(aTensor.zSize() - 1) = 0.5 * aVals3(aTensor.zSize() - 1);
    ...
    for (int z = 0; z < aTensor.zSize(); z++)
      for (int y = 0; y < aTensor.ySize(); y++)
        for (int x = 0; x < aTensor.xSize(); x++)
        {
          ...
          aVals2(aTensor.aSize() - 1) = 0.5 * aVals3(aTensor.aSize() - 1);
    ...
    for (int t = 0; t < aIterations; t++)
    {
      for (int y = 0; y < aData.ySize(); y++)
        for (int x = 0; x < aData.xSize(); x++)
        {
          ...
          T uxy;
          if (uxuy < 0)
            uxy = 2.0 * u11 + u00 + u22 - u10 - u12 - u01 - u21;
          else
            uxy = u10 + u12 + u01 + u21 - 2.0 * u11 - u02 - u20;
