...
// Jia-Baos
// 2024-01-27 13:05
image_t **ReadMatches(const char *matches_name, const int width, const int height)
{
    ...
    matches[1] = NULL;
    matches[2] = NULL;
    image_delete(matches[0]);
    image_delete(matches[1]);
    image_delete(matches[2]);

    image_t *match_x = image_new(width, height);
    image_t *match_y = image_new(width, height);
    image_t *match_z = image_new(width, height);
    image_erase(match_x);
    image_erase(match_y);
    image_erase(match_z);

    FILE *fid;
    fopen_s(&fid, matches_name, "r");
    float x1, x2, y1, y2;
    while (!feof(fid) && fscanf_s(fid, "%f %f %f %f%*[^\n]", &x1, &y1, &x2, &y2) == 4)
    {
...
image_t **parse_options(optical_flow_params_t *params, int argc, char **argv, const int language, const int width, const int height)
{
    ...
    while (1)
    {
        ...
        else if (language == EXE_OPTIONS && !strcmp(argv[current_arg], "-match"))
        {
            ...
            { // if the next argument exists and is not an option, ie must be a filename
