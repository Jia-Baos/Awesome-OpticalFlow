...
void CTensor<T>::normalize(T aMin, T aMax, int aChannel, T aInitialMin, T aInitialMax)
{
  ...
  {
    if (mData[aIndex] > aCurrentMax)
      aCurrentMax = mData[aIndex];
    else if (mData[aIndex] < aCurrentMin)
      aCurrentMin = mData[aIndex];
    aIndex++;
  }
...
void CTensor<T>::rgbToCielab()
{
  for (int y = 0; y < mYSize; y++)
    for (int x = 0; x < mXSize; x++)
    {
      ...
      if (R > 0.0031308)
        R = pow((R + 0.055) * 0.9478673, 2.4);
      else
        R *= 0.077399381;
      if (G > 0.0031308)
        G = pow((G + 0.055) * 0.9478673, 2.4);
      else
        G *= 0.077399381;
      if (B > 0.0031308)
        B = pow((B + 0.055) * 0.9478673, 2.4);
      else
        B *= 0.077399381;
  ...
  for (int y = 0; y < mYSize; y++)
    for (int x = 0; x < mXSize; x++)
    {
      ...
      if (temp > 0.008856)
        ...
        Y = (Y - 0.137931034) * 0.12842;
      temp = X * X * X;
      if (temp > 0.008856)
        X = temp;
      else
        X = (X - 0.137931034) * 0.12842;
      temp = Z * Z * Z;
      if (temp > 0.008856)
        Z = temp;
      else
        Z = (Z - 0.137931034) * 0.12842;
      X *= 0.95047;
      Y *= 1.0;
      Z *= 1.08883;
...
void CTensor<T>::readFromMathematicaFile(const char *aFilename)
{
  ...
  aPos = 0;
  if (aData[aPos] != '{')
    throw EInvalidFileFormat("Mathematica");
  aPos++;
  for (int z = 0; z < mZSize; z++)
  {
    if (aData[aPos] != '{')
      ...
      for (int c = 0; c < cols; c++)
        for (int x = 0; x < mXSize; x++)
        {
          ...
          if (r * cols + c >= mZSize)
            ...
            aHelp = (unsigned char)operator()(x, y, r * cols + c);
...
CVector<T> CTensor<T>::operator()(const float ax, const float ay) const
{
  ...
  int y2 = y1 + 1;
#ifdef _DEBUG
  if (x2 >= mXSize || y2 >= mYSize || x1 < 0 || y1 < 0)
    throw ETensorRangeOverflow(ax, ay, 0);
#endif
  float alphaX = ax - x1;
  float alphaXTrans = 1.0 - alphaX;
  float alphaY = ay - y1;
  float alphaYTrans = 1.0 - alphaY;
  for (int k = 0; k < mZSize; k++)
  {
