...
void CMatrix<T>::normalize(T aMin, T aMax, T aInitialMin, T aInitialMax)
{
  ...
  for (int i = 0; i < aSize; i++)
    if (mData[i] > aCurrentMax)
      aCurrentMax = mData[i];
    else if (mData[i] < aCurrentMin)
      aCurrentMin = mData[i];
  T aTemp = (aCurrentMax - aCurrentMin);
  if (aTemp == 0)
    aTemp = 1;
  else
    aTemp = (aMax - aMin) / aTemp;
  for (int i = 0; i < aSize; i++)
  {
    mData[i] -= aCurrentMin;
    mData[i] *= aTemp;
...
void CMatrix<T>::applySimilarityTransform(CMatrix<T> &aWarped, CMatrix<bool> &aOutside, float tx, float ty, float cx, float cy, float phi, float scale)
{
  ...
  for (int y = 0; y < aWarped.ySize(); y++)
    for (int x = 0; x < aWarped.xSize(); x++, i++)
    {
      ...
      float betaX = 1.0 - alphaX;
      float betaY = 1.0 - alphaY;
      if (x1 < 0 || y1 < 0 || x1 + 1 >= mXSize || y1 + 1 >= mYSize)
        aOutside.data()[i] = true;
      else
      {
        int j = y1 * mXSize + x1;
  ...
  while (!aStack.empty())
  {
    ...
    do
    {
      ...
      if (x > x2 + 1)
        PUSH(y, x2 + 1, x - 1, -dy);
    skip2:
      for (x++; x <= x2 && (operator()(x, y) != aCompValue || aConnected(x, y)); x++)
        ;
      l = x;
    } while (x <= x2);
  }
...
{
#ifdef _DEBUG
  if (aMatrix.xSize() != mXSize)
    throw EIncompatibleMatrices(mXSize, mYSize, aMatrix.xSize(), aMatrix.ySize());
#endif
...
{
  if (mXSize != mYSize)
    throw ENonquadraticMatrix(mXSize, mYSize);
...
template <class T>
void CMatrix<T>::readFromMathematicaFile(const char *aFilename)
{
  using namespace std;
  ...
  while (aStream.get(aChar))
    aData += aChar;
  // Count the number of braces to figure out y-Size of matrix
  int aBraceCount = 0;
  int aPos = 0;
  while ((aPos = aData.find_first_of('{', aPos) + 1) > 0)
    aBraceCount++;
  // Count the number of commas in the first section to figure out xSize of matrix
  int aCommaCount = 0;
  aPos = 0;
  while (aData[aPos] != '}')
  {
  ...
  mYSize = aBraceCount - 1;
  mData = new T[mXSize * mYSize];
  // Analyse file ---------------
  aPos = 0;
  if (aData[aPos] != '{')
    throw EInvalidFileFormatMathematica();
  aPos++;
  for (int y = 0; y < mYSize; y++)
  {
    if (aData[aPos] != '{')
...
#else
      string s = aData.substr(oldPos, aPos);
      istringstream is(s);
#endif
      T aItem;
      is >> aItem;
...
template <class T>
void CMatrix<T>::writeToMathematicaFile(const char *aFilename)
{
  using namespace std;
...
// readFromPDM
template <class T>
void CMatrix<T>::readFromPDM(const char *aFilename)
{
  ...
  aStream >> s;
  if (s != "F")
    throw EInvalidFileFormat("Float PDM");
  // Adjust size of data structure
  delete[] mData;
  mData = new T[mXSize * mYSize];
  // Read data
...
// writeToPDM
template <class T>
void CMatrix<T>::writeToPDM(const char *aFilename)
{
  std::ofstream aStream(aFilename);
  // write header
  aStream << "P9" << std::endl;
  aStream << "SS" << std::endl;
  aStream << mXSize << ' ' << mYSize << std::endl;
  aStream << "F" << std::endl;
  // write data
  for (int i = 0; i < mXSize * mYSize; i++)
  {
...
// readFromIntegerPDM
template <class T>
void CMatrix<T>::readFromIntegerPDM(const char *aFilename)
{
  FILE *aStream;
  aStream = fopen(aFilename, "rb");
  char buffer[200];
  // Find beginning of file (P9)
  fscanf(aStream, "%s", buffer);
  if (std::string(buffer) != "P9")
    throw EInvalidFileFormat("PDM");
  // Remove comments
  while (getc(aStream) == '#')
  {
  ...
  fscanf(aStream, "%s", buffer);
  // Read image size
  fscanf(aStream, "%d", &mXSize);
  fscanf(aStream, "%d", &mYSize);
  // Check integer format
  fscanf(aStream, "%s", buffer);
  if (std::string(buffer) != "I")
    throw EInvalidFileFormat("Integer-PDM");
  // Adjust size of data structure
  delete[] mData;
  mData = new T[mXSize * mYSize];
  getc(aStream);
  ...
  {
    int aDummy;
    fread(&aDummy, sizeof(int), 1, aStream);
    mData[i] = aDummy;
  }
  fclose(aStream);
}
...
// writeToIntegerPDM
template <class T>
void CMatrix<T>::writeToIntegerPDM(const char *aFilename)
{
  FILE *aStream;
  aStream = fopen(aFilename, "wb");
  // write header
  char buffer[200];
  sprintf(buffer, "P9\nSS\n%d %d\nI\n", mXSize, mYSize);
  fwrite(buffer, strlen(buffer), 1, aStream);
  // write data
  for (int i = 0; i < mXSize * mYSize; i++)
  {
    int aDummy = (int)mData[i];
    fwrite(&aDummy, sizeof(int), 1, aStream);
  }
  fclose(aStream);
}
...
// Comparison of two matrices
template <class T>
bool CMatrix<T>::operator==(const CMatrix<T> &aMatrix)
{
