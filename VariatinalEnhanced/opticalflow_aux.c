    ...
    for (j = 0; j < h; j++)
    {
        for (i = 0, o = j * s; i < w; i++, o++)
        {
            if (desc_w->data[o] > 0.0f)
            {
                ...
                float flowscore =
                    abs(sim1->c1[o] - sim2->c1[o2]) + abs(sim1->c2[o] - sim2->c2[o2]) + abs(sim1->c3[o] - sim2->c3[o2]) + grad_weight * (abs(im1dx->c1[o] - im2dx->c1[o2]) + abs(im1dx->c2[o] - im2dx->c2[o2]) + abs(im1dx->c3[o] - im2dx->c3[o2]) + abs(im1dy->c1[o] - im2dy->c1[o2]) + abs(im1dy->c2[o] - im2dy->c2[o2]) + abs(im1dy->c3[o] - im2dy->c3[o2]));
                float t2 = tmp->data[o];
