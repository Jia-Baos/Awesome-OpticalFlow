...
static void prefiltering(float_image *matches, const float_image *edges, const int nn, const float threshold, const float coef_kernel, const int n_thread)
{
    ...
    if (nns != nn + 1)
        fprintf(stderr, "Warning: not enough matches for prefiltering\n");
...
*/
void epic(image_t *flowx, image_t *flowy, const color_image_t *im, const float_image *input_matches,
          float_image *edges, const epic_params_t *params, const int n_thread)
{
...
#pragma omp parallel for num_threads(n_thread)
    ...
    if (params->saliency_th)
    {
        ...
        if (params->verbose)
            printf("Saliency filtering, remaining %d matches\n", matches.ty);
    }
    // consistency filter
    if (params->pref_nn)
    {
        ...
        if (params->verbose)
            printf("Consistenct filter, remaining %d matches\n", matches.ty);
    }

    // prepare variables
    const int nns = MIN(params->nn, matches.ty);
    if (nns < params->nn)
        fprintf(stderr, "Warning: not enough matches for interpolating\n");
    if (params->verbose)
        printf("Computing %d nearest neighbors for each match\n", nns);
    int_image seeds = matches_to_seeds(&matches, n_thread);
    float_image vects = matches_to_vects(&matches, n_thread);
...
#pragma omp parallel for num_threads(n_thread)
    ...
    // interpolation
    if (params->verbose)
        printf("Interpolation of matches using %s\n", params->method);
