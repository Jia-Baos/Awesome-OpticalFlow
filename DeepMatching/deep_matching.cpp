                ...
                res_scale *res)
{
  ...
  for (int i = 0; i < res->assign.tx; i++)
    res->assign.pixels[i] = i;
...
int_image *find_optimal_matchings(matching_pyramid_t &mp, const dm_params_t *params)
{
  ...
  if (params->maxima_mode)
  { // normal process: maxima detection
    ...
    for (unsigned int i = 0; i < mp.size(); i++)
      sc_maxima.pixels[i] = 1; // useless but well
    ...
    for (int i = 0; i < maxima->ty; i++)
      order.pixels[i] = maxima->ty - 1 - i; // last first
  }
  else
  { // we just analyse all cells at the top level
    ...
    *maxima = empty_image(int, 5, (int)LAYERS_SIZE(rmap));
    ...
    // Jia-Baos
    // 2024-01-27 00:05
    qsort_s(order.pixels, maxima->ty, sizeof(int), arg_sort_maxima, maxima->pixels);
#endif
  }
                                    ...
                                    const dm_params_t *params, full_corres_t *corres_out)
{
  ...
  for (i = 0; i < n_scales; i++)
  {
    ...
    if (params->low_mem && size > 1000003)
      size = 1000003; // big prime
...
#pragma omp parallel for schedule(static, 1) num_threads(params->n_thread)
  ...
  for (i = 0; i < n_maxima; i++)
  {
    if (params->verbose && i % 100 == 0)
      std_printf("\rgathering correspondences %d%%...", 100 * i / n_maxima);
  ...
  *res = (float_image){corres, 6, nres};
  ...
  { // save unfiltered correspondences
...
#define mean_dim(shape) ((shape[0] + shape[1]) / 2)
       ...
       sc0++)
  {
    ...
    image_t *scaled_img0 = (scale0 >= 1) ? img0 : image_resize_bilinear_scale(img0, scale0);
         ...
         sc1++)
    {
      ...
      if (sr_params->fast && !(scale0 == 1 || scale1 == 1))
        continue;

      image_t *scaled_img1 = scaled_images1[sc1 - sr_params->min_sc1];
      if (scaled_img1 == NULL)
      {
        scaled_img1 = (scale1 >= 1) ? img1 : image_resize_bilinear_scale(img1, scale1);
        // remember result
...
#define max_dim(img) MAX(img->width, img->height)
        if (max_dim(scaled_img0) >= max_dim(scaled_img1))
        { // first image is always the largest
