...
template <typename T> struct smallest_on_top {
  bool operator()(const T &a, const T &b) const { return a.dis > b.dis; }
};

/* Find nearest neighbors in a weighted directed graph (The matrix must be
 * symmetric !) */
static int find_nn_graph_arr(csr_matrix *graph, int seed, int nmax, int *best,
                             float *dist) {
  ...
  priority_queue<current_t, vector<current_t>, smallest_on_top<current_t>>
      stack;
...
static float arg_sweep(const float_image *cost, float_image *res,
                       int_image *labels, const char x, const char y) {
  int i, j;
...
#define DEFAULT_dt_params(dt_params)                                           \
  ...
  if (!dt_params) {                                                            \
    set_default_dt_params(&tmp_dt_params);                                     \
    dt_params = &tmp_dt_params;                                                \
  }
...
static float weighted_distance_transform(const float_image *cost,
                                         const dt_params_t *dt_params,
                                         float_image *res, int_image *labels) {
  assert(cost && res);
  ASSERT_SAME_SIZE(cost, res);
  if (labels)
    ASSERT_SAME_SIZE(res, labels);
  DEFAULT_dt_params(dt_params) assert(dt_params->min_change >= 0);
  assert(labels);
...
typedef Tint_float_t<long long> long_float_t;
...
static void ngh_labels_to_spmat(int ns, int_image *labels, float_image *dmap,
                                csr_matrix *csr) {
  ...
  typedef unordered_map<long long, border_t> ngh_t;
  ngh_t ngh;
  for (int j = 1; j < ty; j++)
    for (int i = 1; i < tx; i++) {
      int l0 = lab[i + j * tx];
...
/* Compute the neighboring matrix between seeds as well as the closest seed for
 * each pixel */
static void distance_transform_and_graph(const int_image *seeds,
                                         const float_image *cost,
                                         dt_params_t *dt_params,
                                         int_image *labels, float_image *dmap,
                                         csr_matrix *ngh, int n_thread) {
...
/* Compute the closest seeds using a geodesic distance for a subset of points,
   and the assignment of each pixel to the closest seeds best:    output
   containing the closest seeds for each query point dist:    output containing
   the distances to the closest seeds labels:  output containing the assignment
   of each pixel to the closest seed seeds:   2D positions of the seeds cost:
   cost of going throw a pixel (ie that defines the geodesic distance)
    dt_params: distance transform parameters (NULL for default parameters)
    pixels:  2D positions of the query points
*/
void dist_trf_nnfield_subset(int_image *best, float_image *dist,
                             int_image *labels, const int_image *seeds,
...
#pragma omp parallel for num_threads(n_thread)
  ...
  for (int _i = 0; _i < npix; _i++) {
    ...
    for (int j = 0; j < nn; j++)
      dist->pixels[_i * nn + j] =
          d + dis.pixels[s * nn + j]; // distance plus constant
  }
...
/* fit nadaraya watson for a set of seeds
    res:  (output) a nseeds*2 array containing the estimated displacement for
   each seed nnf:   array of size nseeds*nn with index of the nn-closest seed
    dis:   as nnf but containing the distances to the corresponding seed
    vects: 2D vector of the matches
*/
void fit_nadarayawatson(float_image *res, const int_image *nnf,
                        const float_image *dis, const float_image *vects,
                        const int n_thread) {
  const int nn = nnf->tx;
#ifdef USE_OPENMP
#pragma omp parallel for num_threads(n_thread)
...
*/
void apply_nadarayawatson(float_image *newvects, const float_image *seedsvects,
                          const int_image *labels, const int n_thread) {
#ifdef USE_OPENMP
#pragma omp parallel for num_threads(n_thread)
#endif
...
/* fit locally-weighted affine interpolation
    res: (output) a nseeds*6 array containing the estimated affine model for
   each seed nnf:   array of size nseeds*nn with index of the nn-closest seed
    dis:   as nnf but containing the distances to the corresponding seed
    seeds: original point of matches
    vects: 2D vector of the matches
*/
// in addition to the n points, we add 4 matches (close to the current math and
// with a low weight) in order to ensure non-colinearity
#define AFFINE_ADDPOINT(m, v, x, y, wx, wy, c)                                 \
  {                                                                            \
    m[0] = m[8] = (x) * (c);                                                   \
    m[1] = m[9] = (y) * (c);                                                   \
    m[4] = m[11] = (c);                                                        \
...
void fit_localaffine(float_image *res, const int_image *nnf,
                     const float_image *dis, const int_image *seeds,
                     const float_image *vects) {
  const int nn = nnf->tx;
  {
    ...
    for (int i = 0; i < nnf->ty; i++) {
      ...
      for (int j = 0; j < nn; j++) {
        ...
        AFFINE_ADDPOINT(m, v, seeds->pixels[2 * s], seeds->pixels[2 * s + 1],
                        vects->pixels[2 * s], vects->pixels[2 * s + 1], coef);
      }
      float xi = (float)seeds->pixels[2 * i],
            yi = (float)seeds->pixels[2 * i + 1], ui = vects->pixels[2 * i],
            vi = vects->pixels[2 * i + 1];
      ...
      AFFINE_ADDPOINT(m, v, xi, yi - 0.1f, ui, vi, coefi);
      {
        ...
        float work_sz;
        sgels_((char *)"Transposed", &mm, &n, &nrhs, mat, &lda, vec, &ldb,
               &work_sz, &lwork, &info);
        float *work;
        lwork = (int)work_sz;
        work = NEWA(float, lwork);
        sgels_((char *)"Transposed", &mm, &n, &nrhs, mat, &lda, vec, &ldb, work,
               &lwork, &info);
        free(work);
...
/* apply locally-weighted affine interpolation
    ...
    labels:      closest seed for each pixel
*/
void apply_localaffine(float_image *newvects, const float_image *seedsaffine,
                       const int_image *labels, const int n_thread) {
#ifdef USE_OPENMP
#pragma omp parallel for num_threads(n_thread)
#endif
