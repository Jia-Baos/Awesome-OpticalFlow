...
static float arg_sweep(const float_image *cost, float_image *res, int_image *labels, const char x, const char y)
{
  ...
  float *A = res->pixels;           // 仅匹配点有代价（测地线距离），其他区域为0
  int *L = labels->pixels;          // 仅匹配点由标签，其他区域为0
  const float *Cost = cost->pixels; // 由SED计算出来的代价
  // 实现正向反向交替迭代
  ...
  for (j = by; j != ey; j += y)
    for (i = bx; i != ex; i += x)
    {
      ...
      float dt12 = absf(t1 - t2); // 计算测地线距离的绝对值
      C = Cost[i + j * tx];       // 获取当前像素的响应值（edges）
...
/* Compute distance map from a given seeds (in res) and a cost map.
  if labels!=NULL:  labels are propagated along long with distance map
                    (for each pixel, we remember the closest seed)*/
// cost, dt_params, dmap, labels，此处cost等于dmap
...
static void ngh_labels_to_spmat(int ns, int_image *labels, float_image *dmap, csr_matrix *csr)
{
  ...
  const int *lab = labels->pixels; // 标签
  const float *dis = dmap->pixels; // 代价
  ...
  for (int j = 1; j < ty; j++)
    ...
    {
      int l0 = lab[i + j * tx];
      int l1 = lab[i - 1 + j * tx];   // 左侧元素
      int l2 = lab[i + (j - 1) * tx]; // 上侧元素
      if (l0 != l1)
      {
        long long k = key(l0, l1);                       // 获取label
        float d = dis[i + j * tx] + dis[i - 1 + j * tx]; // 获取代价
        ngh[k].add(d);
      }
      if (l0 != l2)
      {
  ...
  {
    const float cost = it->second.get_val(); // 获取代价
    sorted.emplace_back(it->first, cost);    //  标签、代价
    long long symkey = (it->first >> 32) + (it->first << 32);
    sorted.emplace_back(symkey, cost); // add symmetric // 标签、代价
  }
  // sort by (row,col) = (second,first)
  qsort(sorted.data(), sorted.size(), sizeof(long_float_t), cmp_long_float); // 通过对比标签进行排序
...
// seeds, cost, dt_params, labels, &dmap, &ngh, n_thread
static void distance_transform_and_graph(const int_image *seeds, const float_image *cost,
                                         dt_params_t *dt_params, int_image *labels,
                                         float_image *dmap, csr_matrix *ngh, int n_thread)
{
  const int tx = cost->tx; // edges的宽度
  const int ty = cost->ty; // edges的高度
  assert(seeds->tx == 2);
  const int ns = seeds->ty; // 由deepmatching获取到的匹配点的数目
  ...
  // 复制字符 c（一个无符号字符）到参数 str 所指向的字符串的前 n 个字符，0x7F = 127
  // 这里dmap中的所有位置的数值均为0x7F
  memset(dmap->pixels, 0x7F, tx * ty * sizeof(float));

  for (int i = 0; i < ns; i++)
  {
    // 将种子点坐标恢复到原始图像上，初始化匹配点的的标签和代价
    // p可以理解为索引：index = y * edges.cols + x
    int p = seeds->pixels[2 * i] + seeds->pixels[2 * i + 1] * tx;
    // 从左上角开始扫描，label的数目等于匹配点的数目
    // 为每一个种子点分配一个label
    labels->pixels[p] = i;
    // 将edges的数值拷贝到dmap，即所谓的能量
    dmap->pixels[p] = cost->pixels[p];
  }

  // ————————————————————————从此处开始对Geodesic distance进行实现————————————————————————
  // 对距离和标签进行处理
  weighted_distance_transform(cost, dt_params, dmap, labels);

  // compute distances between neighboring seeds
  // ngh中包含了种子点之间的测地线距离
  ngh_labels_to_spmat(ns, labels, dmap, ngh); // ngh，稀疏矩阵，用于存储最近邻
}
...
*/
// &nnf, &dis, &labels, &seeds, edges,  NULL, &seeds, n_thread
                             ...
                             const int_image *pixels, const int n_thread)
{
  ...
  const int ns = seeds->ty; // 种子点的数目
  const int nn = best->tx;  // 最近邻的数目
  assert(dist->tx == nn);
