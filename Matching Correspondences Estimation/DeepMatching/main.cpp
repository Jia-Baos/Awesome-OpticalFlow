...
#define p(msg) std_printf(msg "\n");
  ...
  case EXE_OPTIONS:
    ...
    p("Compute the 'DeepMatching' between two images and print a list of") p(
        "pair-wise point correspondences:") p("  x1 y1 x2 y2 score index ...")
        p("(index refers to the local maximum from which the match was "
          "retrieved)")
            p("Images must be in PPM, PNG or JPG format. Version 1.2.2") break;
  case MATLAB_OPTIONS:
        ...
        p("Compute the 'DeepMatching' between two images.")
            p("Images must be HxWx3 single matrices.")
                p("Options is an optional string argument ('' by default).")
                    p("The function returns a matrix with 6 columns, each row "
                      "being x1 y1 x2 y2 score index.")
                        p("(index refers to the local maximum from which the "
                          "match was retrieved)") p("Version 1.2.2") break;
  case PYTHON_OPTIONS:
    p("matches = deepmatching.deepmatching(image1, image2, options='')")
        p("Compute the 'DeepMatching' between two images.")
            p("Images must be HxWx3 numpy arrays (converted to float32).")
                p("Options is an optional string argument ('' by default).")
                    p("The function returns a numpy array with 6 columns, each "
                      "row being x1 y1 x2 y2 score index.")
                        p("(index refers to the local maximum from which the "
                          "match was retrieved)") p("Version 1.2.2") break;
  }
  p("") p("Options:") p("    -h, --help                 print this message")
      // p("  HOG parameters (low-level pixel descriptor):")
      // p("    -png_settings              (auto) recommended for uncompressed
      // images") p("    -jpg_settings              (auto) recommended for
      // compressed images") p("   in more details: (for fine-tuning)") p("
      // -hog.presm <f=1.0>         prior image smoothing") p("    -hog.midsm
      // <f=1.0>         intermediate HOG smoothing") p("    -hog.sig <f=0.2>
      // sigmoid strength") p("    -hog.postsm <f=1.0>        final
      // HOG-smoothing") p("    -hog.ninth <f=0.3>         robustness to pixel
      // noise (eg. JPEG artifacts)")
      p("") p("  Matching parameters:")
      // p("    -iccv_settings             settings used for the ICCV paper")
      // p("    -improved_settings         (default) supposedly improved
      // settings") p("   in more details: (for fine-tuning)")
      p("    -downscale/-R <n=1>        downsize the input images by a factor "
        "2^n")
      // p("    -overlap <n=999>           use overlapping patches in image1
      // from level n") p("    -subref <n=0>              0: denser sampling or
      // 1: not of image1 patches")
      p("    -ngh_rad <n=0>             if n>0: restrict matching to n pxl "
        "neighborhood")
          p("    -nlpow <f=1.4>             non-linear rectification x := x^f")
      // p("    -maxima_mode <n=0>         0: from all top cells / 1: from local
      // maxima") p("    -min_level <n=2>           skip maxima in levels [0, 1,
      // ..., n-1]")
      p("    -mem <n=1>                 if n>0: optimize memory footprint (bit "
        "unstable)")
      // p("    -scoring_mode <n=1>        type of correspondence scoring mode
      // (0/1)")
      p("") p("  Fully scale & rotation invariant DeepMatching:") p(
          "    if either one of these options is used, then this mode is "
          "activated:")
          p("    -max_scale <factor=5>         max scaling factor")
              p("    -rot_range <from=0> <to=360>  rotation range") p("") p(
                  "  Other parameters:") p("    -resize <width> <height>   to "
                                           "resize input images beforehand")
                  p("    -v                         increase verbosity")
                      p("    -nt <n>                    multi-threading with "
                        "<n> threads") if (language == EXE_OPTIONS) {
...
const char *parse_options(dm_params_t *params, scalerot_params_t *sr_params,
                          ...
                          const int argc, const char **argv, const int language,
...
#define isarg(key) !strcmp(a, key)
    ...
    //  params->desc_params.norm_pixels = atof(argv[current_arg++]);
      ...
      params->desc_params.presmooth_sigma =
          0; // no image smoothing since the image is uncompressed
      params->desc_params.hog_sigmoid = 0.2;
      params->desc_params.mid_smoothing = 1.5;
      params->desc_params.post_smoothing = 1;
      params->desc_params.ninth_dim = 0.1;
    } // low ninth_dim since image PSNR is high
    else if (isarg("-jpg_settings")) {
      params->desc_params.presmooth_sigma =
          1; // smooth the image to remove jpg artifacts
      params->desc_params.hog_sigmoid = 0.2;
      params->desc_params.mid_smoothing = 1.5;
      params->desc_params.post_smoothing = 1;
      params->desc_params.ninth_dim = 0.3;
    ...
    //   params->subsample_ref = 0; // dense patch sampling at every level in
    //   first image params->nlpow = 1.4; params->maxima_mode = 0;
    //   params->low_mem = 1;
    //   params->min_level = 2;
...
int main(int argc, const char **argv) {
  ...
  // // parse options
  // const char* out_filename = parse_options(&params, &sr_params,
  // &use_scalerot, &fx, &fy, argc-current_arg,
  //                                          &argv[current_arg], EXE_OPTIONS,
  //                                          &im1, &im2);

  // // compute deep matching
  // float_image* corres = use_scalerot ?
  //        deep_matching_scale_rot( im1, im2, &params, &sr_params ) :
  //        deep_matching          ( im1, im2, &params, NULL );  // standard
  //        call

  // // save result
  // output_correspondences( out_filename, (corres_t*)corres->pixels,
  // corres->ty, fx, fy );
  ...
  // load images
  image_t *im1 = NULL, *im2 = NULL;
  // std::string cim1_path =
  //     "D:\\CmakeProject\\OpticalFlow\\DeepMatching\\MPI-Sintel\\frame_0029.ppm";
  // std::string cim2_path =
  //     "D:\\CmakeProject\\OpticalFlow\\DeepMatching\\MPI-Sintel\\frame_0030.ppm";
  // const std::string out_filename =
  //     "D:\\CmakeProject\\OpticalFlow\\DeepMatching\\MPI-Sintel\\matches-dm.txt";

  std::string cim1_path = "D:\\Code-VS\\picture\\Demons\\lena3.ppm";
  std::string cim2_path = "D:\\Code-VS\\picture\\Demons\\lena4.ppm";
  const std::string out_filename =
      "D:\\Code-VS\\picture\\Demons\\\\matches-dm.txt";
  color_image_t *cim1 = color_image_load(cim1_path.c_str());
  color_image_t *cim2 = color_image_load(cim2_path.c_str());
  ...
  // save result
  output_correspondences(out_filename.c_str(), (corres_t *)corres->pixels,
                         corres->ty, fx, fy);
