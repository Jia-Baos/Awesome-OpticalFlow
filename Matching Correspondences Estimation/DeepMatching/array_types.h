...
#define ASSERT_ARRAY_ZEROS(arr)                                                                                                                    \
  {                                                                                                                                                \
    int size = arr->tx;                                                                                                                            \
    assert((arr->pixels[0] == 0 && arr->pixels[size / 2] == 0 && arr->pixels[size - 1] == 0) || !"error: matrix " #arr "is supposed to be zeros"); \
  }
...
#define ASSERT_SAME_IMG_SIZE(im1, im2) \
  if (im1 && im2)                      \
    assert(im1->tx == im2->tx && im1->ty == im2->ty);

#define ASSERT_IMAGE_ZEROS
#define ASSERT_IMG_ZEROS(img)                                                                                                                      \
  {                                                                                                                                                \
    int size = img->tx * img->ty;                                                                                                                  \
    assert((img->pixels[0] == 0 && img->pixels[size / 2] == 0 && img->pixels[size - 1] == 0) || !"error: matrix " #img "is supposed to be zeros"); \
  }
...
#define ASSERT_SAME_CUBE_SIZE(im1, im2) \
  if ((im1) && (im2))                   \
    assert((im1)->tx == (im2)->tx && (im1)->ty == (im2)->ty && (im1)->tz == (im2)->tz);

#define ASSERT_CUBE_ZEROS(img)                                                                                                                     \
  {                                                                                                                                                \
    int size = img->tx * img->ty * img->tz;                                                                                                        \
    assert((img->pixels[0] == 0 && img->pixels[size / 2] == 0 && img->pixels[size - 1] == 0) || !"error: matrix " #img "is supposed to be zeros"); \
  }
